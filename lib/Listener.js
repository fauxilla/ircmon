...
export default class Listener {
  ...
  constructor (listener, opt) {
    if (!opt.trackers[listener.tracker]) {
      throw new Error(`no options specified for ${listener.tracker}`)
    }
    Object.assign(this, listener, opt.trackers[listener.tracker])
    if (!this.tracker) throw new Error('tracker name is required')
    this.opt = opt
  ...
  async init () {
    ...
    this.tracker = await loadTracker(tracker)
    const {
      network,
      host,
      port,
  ...
  parse (message, from) {
    ...
    torrent.uri = format(
      uriTemplate,
      Object.assign({}, torrent, this.tracker)
    )
  ...
  async handler (from, to, message) {
    ...
    try {
      ...
      check(torrent, this)
      info('passed all checks')
      await this.downloadTorrent(torrent)
    } catch (err) {
      if (err instanceof Err) {
        this.checkError = err
        info(`ignore: ${err.message}`)
        return
      }
      error({ meta: torrent }, 'error handling torrent')
      throw err
    }
  ...
  async downloadTorrent (torrent) {
    if (this.opt.deluge.enabled) await deluge(torrent, this)
    if (this.opt.blackhole.enabled) await blackHole(torrent, this)
  }
